$(document).ready(function(){
    ...
    /* 
       ...
       하나의 버튼이 두가지 기능 - 정지와 재생 기능을 구분하는 값.... 
       btn_stop 버튼에 play클래스가 없으면 일시정지
       btn_stop 버튼에 play클래스가 있으면 재생 
    */
    $('.visual .popup .btn_wrap .btn_stop').on('click', function(){
    ...
    function fabcScroll(){ //스크롤 값을 계산해서, fabric의 이미지를 움직일 함수
        /* 스크롤값을 요소의 위치값으로 변경해서 스타일을 적용 효과를 줄 요소가 화면의 하단
        에 등장하기 시작했을 때부터 스크를 한 값을 새로 계산해서 해당 요소에 줘야 해당 요
        소가 화면 어디에 배치되어 있든 자연스럽게 패럴럭스 효과를 줄 수 있음 */
