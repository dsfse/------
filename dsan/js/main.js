    ...
    */
    ...
    function stepResize(){
        ...
        if(winW <= 640){ // 모바일일때만 들어가는 효과
            ...
            모바일 -> pc로 전환할때 모바일에서 준 넓이를 다시 pc의 넓이로 초기화 해야함
            */
    ...
    $('.step .ctrl button.prev').on('click',function(){
        /*  
            ...
            5 -> 4 -> 3 -> 2 -> 1 -> 0이하면 안함
            1번이 보일때 ol의 left는 0
            2번이 보일때 ol의 left는 (li의 넓이 + li의 여백) 값을 음수로 줌
            3번이 보일때 ol의 left는 (li의 넓이 + li의 여백) *2 값을 음수로 줌
            4번이 보일때 ol의 left는 (li의 넓이 + li의 여백) *3 값을 음수로 줌
    ...
    /* 
        family site 
        footer .family를 클릭하면 fa_open 클래스를 추가/삭제
    */
    ...
    /* 
        브라우저가 스크롤을 할때 스크롤 값이 0보다 크면
        header에 fixed라는 클래스를 줄 예정
        스크롤 값이 0이되면 header의 fixed 클래스를 삭제
    */
