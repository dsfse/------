$(document).ready(function(){
    ...
    /*
        ...
        1. class명을 조정해서 상태에 따라 완전히 선택자를 다르게 할 수 있는 
           이벤트를 두번 주는 경우
           .popup .btn_stop   .popup .btn_stop.play
        2. if문으로 현재 play상태인지 stop상태인지 구분해서 적용하는 방법
           하나의 이벤트로 

     */
    $('.popup .btn_stop').on('click', function(){
        if($(this).hasClass('play') == true){ //만약에 현재 play 클래스를 가지고 있다면 (일시정지)
            swiper.autoplay.start();
            $(this).removeClass('play'); /* 재생 기능 */
            $(this).text('일시정지');  
        }else{ // 재생중
